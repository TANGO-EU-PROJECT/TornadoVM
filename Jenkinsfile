...
void buildAndTest(String JDK, String tornadoProfile) {
    ...
    stage('TornadoVM Tests') {
        timeout(time: 12, unit: 'MINUTES') {
            sh 'tornado --devices'
            sh '${TORNADO_DOCKER_SCRIPT}/polyglotImages/polyglot-graalpy/tornadovm-polyglot-intel.sh tornado-test -V uk.ac.manchester.tornado.unittests.profiler.TestProfiler'
        }
    }
    stage("TornadoVM Integration Tests (Java-Python)") {
        timeout(time: 12, unit: 'MINUTES') {
            sh 'cd ${TORNADO_DOCKER_SCRIPT} && ${TORNADO_DOCKER_SCRIPT}/polyglotImages/polyglot-graalpy/tornadovm-polyglot-intel.sh tornado --truffle python example/polyglot-examples/kmeans.py'
        }
    }
}
